...
namespace BEPUphysics.CollisionShapes
{
    ...
    public class TerrainShape : CollisionShape
    {
        ...
        ///<summary>
        /// Constructs the bounding capsule of the terrain given a transform.
        ///</summary>
        ///<param name="transform">Transform to apply to the terrain during the bounding capsule calculation.</param>
        ///<param name="boundingBox">Bounding capsule of the terrain shape when transformed.</param>
        ...
        ///<summary>
        /// Gets overlapped triangles with the terrain shape with a bounding capsule in the local space of the shape.
        ///</summary>
        ///<param name="localBoundingBox">Bounding capsule in the local space of the terrain shape.</param>
        ///<param name="overlappedElements">Indices of triangles whose bounding boxes overlap the input bounding capsule. Encoded as 2 * (quadRowIndex * terrainWidthInQuads + quadColumnIndex) + isFirstTriangleOfQuad ? 0 : 1, where isFirstTriangleOfQuad refers to which of the two triangles in a quad is being requested. Matches the input of the TerrainShape.GetTriangle function.</param>
        ...
        public bool GetOverlaps<T>(BoundingBox localBoundingBox, ref T overlappedElements) where T : IList<int> //Designed to work with value type ILists, hence anti-boxing interface constraint and ref.
        {
            ...
            for (int i = minX; i <= maxX; i++)
            {
                for (int j = minY; j <= maxY; j++)
                {
                    ...
                    //Now the local bounding capsule is very likely intersecting those of the triangles.
