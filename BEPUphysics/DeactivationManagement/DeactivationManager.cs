            ...
            if (member.connections.Count > 0)
            {
                for (int i = 0; i < member.connections.Count; i++)
                {
                    ...
                    if (representativeA == null)
                    {
                        ...
                        //no member had a simulation island.  Consider removing a dynamic capsule from the space
                        //while it sits on a kinematic capsule.  Neither object has a simulation island.
